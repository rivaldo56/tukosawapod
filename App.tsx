...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadEpisodes = async () => {
      const fetchedEpisodes = await fetchEpisodes('https://anchor.fm/s/106338194/podcast/rss');
        ...
        <div className="order-2 md:order-1 flex flex-col items-center md:items-start text-center md:text-left w-full md:w-5/12 z-20">
          ...
          >
            {(() => {
              ...
              return (
                <>
                  {epPrefix && <span className="block">{epPrefix}</span>}
                  {firstHalf && <span className="block">{firstHalf}</span>}
                  {secondHalf && <span className="block">{secondHalf}</span>}
                </>
              );
            })()}
          </motion.h1>
        ...
        <div className="container mx-auto max-w-6xl relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {[
              ...
              "Nairobi love, messy friendships, heartbreaks that humbled you, drama you survived by accident, and those 2 a.m. decisions that still haunt you.",
              "Chaotic chemistry between Jay & Ghost that makes even serious topics hilarious.",
              "Short, addictive episodes you can binge anywhere: matatu, during class (we won’t tell), gym, lunch break, or when you’re overthinking in bed.",
              "Anonymous hosts = zero filters, more truth, more madness, more “did they really say that?” moments.",
